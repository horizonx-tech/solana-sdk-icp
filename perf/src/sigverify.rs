...
mod tests {
    ...
    #[tokio::test]
    async fn test_system_transaction_layout() {
        let tx = test_tx().await;
        let tx_bytes = serialize(&tx).unwrap();
    ...
    #[tokio::test]
    async fn test_pubkey_too_small() {
        solana_logger::setup();
    ...
    async fn test_pubkey_len() {
        ...
        info!("tx: {:?}", tx);
        let sig = keypair1.try_sign_message(&tx.message_data()).await.unwrap();
        tx.signatures = vec![sig; NUM_SIG];
    ...
    #[tokio::test]
    async fn test_really_large_sig_len() {
        let tx = test_tx().await;
        let mut packet = Packet::from_data(None, tx).unwrap();
    ...
    #[tokio::test]
    async fn test_invalid_pubkey_len() {
        let tx = test_tx().await;
        let mut packet = Packet::from_data(None, tx).unwrap();
    ...
    #[tokio::test]
    async fn test_versioned_message() {
        let tx = test_tx().await;
        let mut packet = Packet::from_data(None, tx).unwrap();
    ...
    #[tokio::test]
    async fn test_system_transaction_data_layout() {
        let mut tx0 = test_tx().await;
    ...
    #[tokio::test]
    async fn test_get_packet_offsets() {
        assert_eq!(
            get_packet_offsets_from_tx(test_tx().await, 0),
            PacketOffsets::new(1, 1, 64, 4, 2)
        );
        assert_eq!(
            get_packet_offsets_from_tx(test_tx().await, 100),
            PacketOffsets::new(1, 1, 64, 4, 2)
        );

        // Ensure we're not indexing packet by the `current_offset` parameter.
        assert_eq!(
            get_packet_offsets_from_tx(test_tx().await, 1_000_000),
            PacketOffsets::new(1, 1, 64, 4, 2)
        );

        // Ensure we're returning sig_len, not sig_size.
        assert_eq!(
            get_packet_offsets_from_tx(test_multisig_tx().await, 0),
    ...
    async fn test_verify_n(n: usize, modify_data: bool) {
        let tx = test_tx().await;
        let mut packet = Packet::from_data(None, tx).unwrap();
    ...
    #[tokio::test]
    async fn test_verify_tampered_sig_len() {
        let mut tx = test_tx().await;
    ...
    #[tokio::test]
    async fn test_verify_zero() {
        test_verify_n(0, false).await;
    }

    #[tokio::test]
    async fn test_verify_one() {
        test_verify_n(1, false).await;
    }

    #[tokio::test]
    async fn test_verify_seventy_one() {
        test_verify_n(71, false).await;
    }

    #[tokio::test]
    async fn test_verify_medium_pass() {
        test_verify_n(VERIFY_PACKET_CHUNK_SIZE, false).await;
    }

    #[tokio::test]
    async fn test_verify_large_pass() {
        test_verify_n(VERIFY_PACKET_CHUNK_SIZE * get_thread_count(), false).await;
    }

    #[tokio::test]
    async fn test_verify_medium_fail() {
        test_verify_n(VERIFY_PACKET_CHUNK_SIZE, true).await;
    }

    #[tokio::test]
    async fn test_verify_large_fail() {
        test_verify_n(VERIFY_PACKET_CHUNK_SIZE * get_thread_count(), true).await;
    }

    #[tokio::test]
    async fn test_verify_multisig() {
        solana_logger::setup();

        let tx = test_multisig_tx().await;
        let mut packet = Packet::from_data(None, tx).unwrap();
    ...
    //#[tokio::test]
    //async fn test_is_simple_vote_transaction() {
    //    solana_logger::setup();
    //    let mut rng = rand::thread_rng();
    ...
    //    {
    //        let mut tx = new_test_vote_tx(&mut rng).await;
    //        tx.message.instructions[0].data = vec![1, 2, 3];
    //        let mut packet = Packet::from_data(None, tx).unwrap();
    //        let packet_offsets = do_get_packet_offsets(&packet, 0).unwrap();
    //        check_for_simple_vote_transaction(&mut packet, &packet_offsets, 0).ok();
    ...
    //#[tokio::test]
    //async fn test_is_simple_vote_transaction_with_offsets() {
    //    solana_logger::setup();
    //    let mut rng = rand::thread_rng();
    ...
    //        batch.push(Packet::from_data(None, test_tx().await).unwrap());
    //        let tx = new_test_vote_tx(&mut rng).await;
