...
pub async fn test_tx() -> Transaction {
    let keypair1 = Keypair::new();
    let pubkey1 = keypair1.pubkey();
    let zero = Hash::default();
    system_transaction::transfer(&keypair1, &pubkey1, 42, zero).await
}

pub async fn test_invalid_tx() -> Transaction {
    let mut tx = test_tx().await;
    tx.signatures = vec![Transaction::get_invalid_signature()];
    tx
}

pub async fn test_multisig_tx() -> Transaction {
...
//pub async fn new_test_vote_tx<R>(rng: &mut R) -> Transaction
//where
//    R: CryptoRng + RngCore,
//{
